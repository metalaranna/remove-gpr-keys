...
#
# Please make copy of this file inside of your repo right before execution
# Please delete this file after execution and before commit 
...
for keyfilename in $CURRENT_DIR/.git-crypt/keys/default/0/*gpg; do
    ...
    if [[ $@ == *$key* ]]; then
	    echo deleting $key
        rm $CURRENT_DIR/.git-crypt/keys/default/0/$key.gpg      
        continue;
    fi
    set +x
